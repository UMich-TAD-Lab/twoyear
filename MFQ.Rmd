...
```{r Basic stuff}
mean(mfq$MFQ_C_Sum)
mean(mfq$mfq_total_sum)

sd(mfq$MFQ_C_Sum)
sd(mfq$mfq_total_sum)

female <- mfq[ which(mfq$gender=='F'), ]
male <- mfq[ which(mfq$gender=='M'), ]

hist(female$MFQ_C_Sum)
hist(female$mfq_total_sum)

hist(male$MFQ_C_Sum)
hist(male$mfq_total_sum)
...
ggplot(mfq, aes(MFQ_C_Sum, mfq_total_sum)) + geom_point(fill="blue") +
  ggtitle("MFQ Wave 15 Scatterplot) + xlab("MFQ Original Total Score") + ylab("MFQ Two Year Total Score")
```


```{r echo=FALSE}
cor.test(mfq$MFQ_C_Sum, mfq$mfq_total_sum) #Is there a correlation between the old and new scores?
cor.test(female$MFQ_C_Sum, female$mfq_total_sum)
cor.test(male$MFQ_C_Sum, male$mfq_total_sum)
```

```{r echo=FALSE}
t.test(mfq$MFQ_C_Sum, mfq$mfq_total_sum) #Is there a significant difference between these two scores?
